...
export default function AIRecommendedRecipes() {
  ...
  const generateRecommendations = async () => {
    ...
    if (now - lastRequestTime < 30000) {
      const remainingSeconds = Math.ceil((30000 - (now - lastRequestTime)) / 1000);
      setError(`Please wait ${remainingSeconds} seconds before requesting new recommendations.`);
      return;
    }
...
`;
      ...
      // Map profile.aiModel to actual OpenAI model names
      let actualModel = 'gpt-3.5-turbo'; // default fallback
      ...
      console.log('Profile aiModel:', profile.aiModel);
      ...
      if (err.message?.includes('429')) {
        ...
        setError('Invalid OpenAI API key. Please check your API key in Profile settings.');
      } else if (err.message?.includes('402')) {
        setError('OpenAI quota exceeded. Please check your OpenAI account billing.');
      } else {
